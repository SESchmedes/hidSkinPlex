...
dat.all <- read.table("/mnt/blsm/sarah/targetedstudy/analysis/metaphlan_results/mpileup_results/nonuniversal/all_markerND_fv_R_gteq25.txt", header=T, sep="\t")
...
plot.all <- ggplot(df.all, aes(x=PC1, y=PC2, color=Bodysite, shape=Bodysite)) + geom_point(size=3) + xlab("PC1 (36% variance explained)") + ylab("PC2 (16% variance explained)")

dat.fb <- read.table("/mnt/blsm/sarah/targetedstudy/analysis/metaphlan_results/mpileup_results/Fb_markerND_fv_R_gteq200.txt", header=T, sep="\t")
...
plot.fb <- ggplot(df.fb, aes(x=PC1, y=PC2, color=Individual)) + geom_point(size=3) + xlab("PC1 (48% variance explained)") + ylab("PC2 (15% variance explained)") + theme(legend.position = "none")

dat.hp <- read.table("/mnt/blsm/sarah/targetedstudy/analysis/metaphlan_results/mpileup_results/Hp_markerND_fv_R_gteq200.txt", header=T, sep="\t")
...
plot.hp <- ggplot(df.hp, aes(x=PC1, y=PC2, color=Individual)) + geom_point(size=3) + xlab("PC1 (35% variance explained)") + ylab("PC2 (28% variance explained)")

dat.mb <- read.table("/mnt/blsm/sarah/targetedstudy/analysis/metaphlan_results/mpileup_results/nonuniversal/Mb_markerND_fv_R_gteq150.txt", header=T, sep="\t")
...
plot.mb <- ggplot(df.mb, aes(x=PC1, y=PC2, color=Individual)) + geom_point(size=3) + xlab("PC1 (39% variance explained)") + ylab("PC2 (16% variance explained)") + theme(legend.position = "none")
...
ggsave("/mnt/blsm/sarah/OhData/metaphlan_results/analysis/Ac_Ch_markerND_shared50-10_gteq5_FS_PCA_figureforpaper.png", plot=plot, scale = 1, width=30, height=20, units=c("cm"),dpi=300, limitsize=T)
