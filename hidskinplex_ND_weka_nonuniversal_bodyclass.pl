...
my $pilepath = "/mnt/blsm/sarah/targetedstudy/analysis/metaphlan_results/mpileup_results/nonuniversal";
my (%ampsize, %amppaths);
...
my $bodysite = "bodyclass";
#my %bodysortednames;
my @bodysortednames;
#if (! -f "$pilepath/$bodysite\_ND.txt") {
#    if (system("perl /home/sarah/src/hidskinplex_pileparseMarkerND_nonuniversal.pl -bodysite $bodysite")) {
#	die "problem executing hidskinplex_pileparseMarkerND_nonuniversal.pl. $!\n";
#    }
#}
...
foreach my $threshold (@thresholds) {
    open IN, "<$pilepath/all_ND.txt" or die "Could not open ND.txt file for reading\n";
    my %markercov;
    my %samplemarkercov;
    my %sampleND;
...
#		    $samplemarkerlengthcov{$sample}++;
    ...
    print FV "$markerstring\tBodysite\n";
    
    foreach my $sample (@bodysortednames) {
	my $out;
	foreach my $marker (@markers) { #changed to ampsize from markercov since not common
	    if (! exists $sampleND{$marker}{$sample}) {
		$sampleND{$marker}{$sample} = 0;
	    }
	    my $ND = $sampleND{$marker}{$sample}/$ampsize{$marker};	    
	    $out .= "$ND\t";
	}
	my ($sampleid, $body, $time, $rep) = split /-/, $sample;
	$out .= $body;
...
#Run R script to create .arff
    if(system("Rscript /home/sarah/src/hidskinplex_markerND_fv_to_arff_nonuniversal_bodyclass.R --bodysite $bodysite --threshold $threshold")) {
	die "Error executing rscript to create .arff file\n";
    }
    
#weka commands here
#Logistic
    if(system("java -Xmx8g -classpath /home/sarah/weka-3-8-1/weka.jar weka.classifiers.functions.Logistic -R 1.0E-8 -M -1 -num-decimal-places 4 -x 10 -t $pilepath/$bodysite\_markerND_fv_gteq$threshold\.arff > $pilepath/$bodysite\_weka_logistic_markerND_gteq$threshold\.txt")) {
	die "Error executing java and weka logistic. $!\n";
    }
    
#KNN
    if(system("java -Xmx8g -classpath /home/sarah/weka-3-8-1/weka.jar weka.classifiers.lazy.IBk -K 1 -x 10 -t $pilepath/$bodysite\_markerND_fv_gteq$threshold\.arff > $pilepath/$bodysite\_weka_knn_markerND_gteq$threshold\.txt")) {
	die "Error executing java and weka knn. $!\n";
    }
    
#AttSelectLog
    if(system("java -Xmx8g -classpath /home/sarah/weka-3-8-1/weka.jar weka.classifiers.meta.AttributeSelectedClassifier -x 10 -t $pilepath/$bodysite\_markerND_fv_gteq$threshold\.arff -W weka.classifiers.functions.Logistic -- -R 1.0E-8 -M -1 -num-decimal-places 4 > $pilepath/$bodysite\_weka_attselectLogistic_markerND_gteq$threshold\.txt")) {
	die "Error executing java and weka attselect logistic. $!\n";
    }
    
#AttSelectKNN
    if(system("java -Xmx8g -classpath /home/sarah/weka-3-8-1/weka.jar weka.classifiers.meta.AttributeSelectedClassifier -x 10 -t $pilepath/$bodysite\_markerND_fv_gteq$threshold\.arff -W weka.classifiers.lazy.IBk -- -K 1 > $pilepath/$bodysite\_weka_attselectknn_markerND_gteq$threshold\.txt")) {
...
#Prepare line out
    open RESULTS, ">/mnt/blsm/sarah/targetedstudy/analysis/metaphlan_results/mpileup_results/nonuniversal/$bodysite\_markerND_weka_compare_MASTER_gteq$threshold\.txt" or die "Cannot open output for writing. $!\n";
    ...
    open SHORT, ">/mnt/blsm/sarah/targetedstudy/analysis/metaphlan_results/mpileup_results/nonuniversal/$bodysite\_markerND_weka_compare_short_gteq$threshold\.txt" or die "Cannot open short output for writing. $!\n";
	...
	foreach my $sample (@bodysortednames) { #change to ampsize
	    if (! exists $samplemarkercov{$marker}{$sample}) {
		$samplemarkercov{$marker}{$sample} = 0;
	    }
	    my $avgsitecov = $samplemarkercov{$marker}{$sample}/$ampsize{$marker};
